<template>
    <div style='width:50%' >
        <el-form :model="ruleForm"  ref="ruleForm"  label-width="100px" status-icon :rules="rules">
...
export default {
	data () {
		return {
			...
			rules: {
				isLoveCV: [{required: true, message: '请输入2', trigger: 'change'}],
				desc: [{required: true, message: '请输入2', trigger: 'change'}],
				radio1: [{required: true, message: '请输入', trigger: 'change'}],
				name: ''
			}
		};
	},
	methods: {
		...
		change(e){
			if(e === 0){
				this.rules.name = {required: true, message: '请输入工地名称', trigger: 'blur'};
			}else {
				this.rules.name = '';
			}
		}
