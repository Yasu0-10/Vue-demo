...
<script>
	export default {
			...
			return {
				skuListInfo: [
					...
					},
					{
						id: 8,
						name: '大客户', // 名称
						storeIds: [1, 2, 3, 4], // 适用门店
						storeIdInfo: '[1, 2, 3, 4]',
						productType: '6',
						productInfo: '暑假托', // 适用产品
						feeType: '3',
						mergeId: 1,
						feeTypeInfo: '学杂费', // 费用类型
						billing: '月/季/年制度' // 计费方法
					}
				],
				typeNameArr: [],
				typeNamePos: 0,
		...
		methods: {
			merageInit() {
				this.typeNameArr = [];// 第一列合并时预计返回的数组。[2,1]这种
				this.typeNamePos = 0;// 预计合并的行数
				this.storeArr = [];
				this.storePos = 0;
				this.feeArr = [];
				this.feePos = 0;
			...
			merage() {
				...
				for (let i = 0; i < this.skuListInfo.length; i += 1) {
					if (i === 0) {
						// 第一行必须存在
						this.typeNameArr.push(1);// 名称
						this.typeNamePos = 0;
						this.storeArr.push(1);// 适用
						this.storePos = 0;
						this.feeArr.push(1);// 费用
						this.feePos = 0;
					} else { // 2345。。行
						// 判断当前行元素与上一个元素是否相同,eg：this.typeNamePos 是 this.typeNameArr序号
						// 第一列
			...
			objectSpanMethod({ row, column, rowIndex, columnIndex }) {
				if (columnIndex === 0) {
					...
					const _row = this.typeNameArr[rowIndex];
