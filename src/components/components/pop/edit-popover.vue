  ...
  <el-popover
    ...
    <el-form
      ...
      <el-form-item label-width="0" prop="val">
        <el-input v-if="type==='input'" v-model.trim="form.val" size="mini" :placeholder="placeholder"/>
        <el-input v-else-if="type==='textarea'" v-model="form.val" type="textarea" :rows="2" size="mini" resize="none" :placeholder="placeholder"/>
        <el-input-number v-else v-model="form.val" :min="0" size="mini" controls-position="right" :placeholder="placeholder"/>
      </el-form-item>
    </el-form>
...
export default {
	...
	data () {
		...
		return {
			...
			rules: {
				val: [{validator: validateNum, trigger: 'blur'}]
			}
		};
	},
	props: {
