...
var recordMaxTime = 180;
var questionTitleEditor = null,
	combTitleEditor = null;
...
$(document).ready( function() {
    ...
    $("#saveAndAddButton").click(function(e) {
        var type=$("#asyncForm input[name=type]").val();
        if(type==6){
            var classification = $("#subForm input[name=classification]").val();
            if(classification == "0"){
    ...
    $('#createExam').click(function () {
		window.location.href = getAdminUrlPrefix + '/examadmin/admin/paper_add_new';
    });
...
function resetPage() {
	...
	if(type=="1"||type=="2"){
		...
		for (var i = key_length; i < 2; i++) {
			html = '<div class="keyList">'+
					...
					'    </div><a href="javascript:void(0);" class="removeKey icons8-delete"></a>'+
					'</div>';
	...
	$(".questionPanel .questionList, #keyBlock, #preview").empty();
	$(".questions_add").html("<p><br/></p>");
...
function initialEditor(o) {
	...
	editor.config.uploadImgUrl = '/baseinfo/admin/upload/?userRole=admin&action=uploadimage';
	...
	if($(o).attr("id") == "questionEditor" || $(o).attr("id") == "combEditor"){
		editor.$editorContainer.find(".menu-group:eq(2)").append('<div onclick="showRecordModalFn(event)" class="menu-item clearfix clearfix">' +
			'<a href="#" tabindex="-1">' +
			'<i class="icon-m_exam_record"></i>' +
			'</a>' +
			'<a href="#" tabindex="-1" class="selected" style="display: none;">' +
			'<i class="icon-m_exam_record"></i>' +
			'</a>' +
			'<div class="menu-tip" style="width: 60px; ">上传录音</div>' +
			'</div>');

		if($(o).attr("id") == "questionEditor"){
			questionTitleEditor = editor;
		}else {
			combEditor = editor;
		}
	}
    ...
    if (companyId && companyId!=undefined) {
        var data = {
            ...
            "instructions": {
                ...
                "operationDetail": '使用 [手动录入] 成功录入 [1] 道题'
            },
        };
...
function filterContentIsEmpty(str) {
	...
	var regAudio = /<audio(.*?)audio>/gi;
    var filterStr;

    if(str === ' '){
		return true;
	}
	if(regImg.test(str)||regTable.test(str)||regFrame.test(str)||regAudio.test(str)){
    	return false;
	}
...
// 显示录音弹窗
function showRecordModalFn() {
	$("#recordModal").modal();
}

// 上传录音
function uploadRecordFileFn(file) {
	uploadFile(file, uploadRecordCallbackFn);
}

// 发送上传录音的请求
function uploadFile(file, callbackFn) {
	var data = new FormData();

	data.append('upfile', file);

	$.ajax({
		url: '/baseinfo/admin/upload/?userRole=admin&action=uploadvideo',
		type: 'POST',
		data: data,
		dataType: 'JSON',
		cache: false,
...
// 上传完录音回调
function uploadRecordCallbackFn(audio) {
	var e = window.event;

	$("#recordModal .delRecord").trigger("click");
	$("#recordModal").modal("hide");

	// 在题目中插入一个音频标签
	if(window.event){
		if ($(".wangEditor-container.active").find(".questions_add.wangEditor-txt").attr("id") == "combEditor") {
			combEditor.command(window.event, 'insertHtml','<img class="audio-temp-img" temp_src="'+audio.url+'" data-alt="'+audio.original+'">');
		}else {
			questionTitleEditor.command(window.event, 'insertHtml','<img class="audio-temp-img" temp_src="'+audio.url+'" data-alt="'+audio.original+'">');
		}
	} else {
		$(".wangEditor-container.active .questions_add").append('<img class="audio-temp-img" temp_src="'+audio.url+'" data-alt="'+audio.original+'">');
	}
	$(".wangEditor-container.active .questions_add").blur();
	$(".wangEditor-container.active .questions_add").focus();
}

// 录音弹窗关闭提示
$("#recordModal").on('hide.bs.modal', function(e){
	if(!$("#recordModal .status-container").hasClass("initial")){
		e.preventDefault();

		BootstrapDialog.show({
			title: "",
			message: "请确认是否离开录音，确认离开则当前录音内容将自动清除，此操作不可逆",
			buttons: [{
				label: "否",
				...
				action: function(dialogItself) {
					dialogItself.close();
					$("#recordModal .stopRecord").trigger("click");
					$("#recordModal .delRecord").trigger("click");
					$("#recordModal").modal("hide");
					return true;
				}
